...
class Filters extends React.Component {
    createProject = (project) => {
        return (
            <ListGroup.Item action key={project} href={`#${project}`} active = {this.props.activeFilter === project} onClick = {() => {this.props.filterTasks('filter-project', project)}}>{project}</ListGroup.Item>
        );
    }
        ...
        return (
            <>
                <ListGroup  variant="flush">
                    ...
                    <ListGroup.Item action key = "#week" active = {this.props.activeFilter === "Next Week"} id = "filter-week" onClick = {() => {this.props.filterTasks('filter-week');}}>Next 7 Days</ListGroup.Item>
